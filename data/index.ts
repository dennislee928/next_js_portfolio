export const navItems = [
  { name: "About", link: "#about" },
  { name: "Stats", link: "#stats" },
  { name: "Tech Stack", link: "#techstack" },
  { name: "Projects", link: "#projects" },
  { name: "Certifications", link: "#certifications" },
  { name: "Contact", link: "#contact" },
];

export const gridItems = [
  {
    id: 3,
    title: "",
    description: "",
    className:
      "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh] color:black",
    imgClassName: "w-full h-full",
    titleClassName: "justify-en color:black",
    img: "/b1.svg",
    spareImg: "",
  },
  {
    id: 2,
    title: "",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "",
    spareImg: "",
  },
  {
    id: 4,
    title: "",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },
  {
    id: 4,
    title: "",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },
  {
    id: 5,
    title: "",
    description: "",
    className: "md:col-span-3 md:row-span-2",
    imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "/gcp.svg",
    spareImg: "/grid.svg",
  },
  {
    id: 6,
    title: "",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-center md:max-w-full max-w-60 text-center",
    img: "",
    spareImg: "",
  },
];

export const projects = [
  {
    id: 1,
    title: "",
    des: "",
    img: "/carbon.png",
    iconLists: ["/re.svg", "/tail.svg", "/three.svg"],
    link: "https://github.com/dennislee928/carboon-trade-backend",
    demoVideo: "https://youtu.be/_iW3o3_Lvzs",
  },
  {
    id: 2,
    title: "",
    des: "",
    img: "/web3.png",
    iconLists: ["/re.svg", "/tail.svg"],
    link: "https://web3.dennisleehappy.org/",
    demoVideo: "https://web3.dennisleehappy.org/"
  },
  {
    id: 3,
    title: "",
    des: "",
    img: "/p3.svg",
    iconLists: ["/vue.svg", "/nuxt3.svg", "/ts.svg"],
    link: "https://github.com/dennislee928/Cloudflare_Developer_Labs",
    demoVideo: "https://youtu.be/vWDmq0GiCo4",
  },
  {
    id: 4,
    title: "",
    des: "",
    img: "/yara.png",
    iconLists: ["/ts.svg"],
    link: "https://github.com/dennislee928/firmware-research-demo",
    demoVideo: "https://youtu.be/rHVcB-mxKB8",
  },
  {
    id: 5,
    title: "",
    des: "",
    img: "/p5.svg",
    iconLists: ["/re.svg", "/tail.svg", "/three.svg"],
    link: "https://github.com/dennislee928/side-project-1-backend",
  },
  {
    id: 6,
    title: "",
    des: "",
    img: "/p6.svg",
    iconLists: ["/re.svg"],
    link: "https://github.com/dennislee928/firmware-research-demo/tree/tsse",
    demoVideo: "https://youtu.be/43sSu1Ve55s",
  },
  {
    id: 7,
    title: "",
    des: "",
    img: "/bot.png",
    iconLists: ["/re.svg", "/tail.svg"],
    link: "https://github.com/dennislee928/fundbot-go",
    demoVideo:"https://github.com/dennislee928/fundbot-go"
  },
  {
    id: 8,
    title: "",
    des: "",
    img: "/p2.svg",
    iconLists: ["/vue.svg", "/nuxt3.svg"],
    link: "https://shipeng-dev.dennisleehappy.org/login",
    demoVideo: "https://shipeng-dev.dennisleehappy.org/login"
  },
  {
    id: 9,
    title: "",
    des: "",
    img: "/essentia.png",
    iconLists: ["/re.svg", "/three.svg"],
    link: "https://essentia.dennisleehappy.org/",
    demoVideo: "https://essentia.dennisleehappy.org/"
  },
  {
    id: 10,
    title: "",
    des: "",
    img: "/ielts.jpg",
    iconLists: ["/re.svg", "/tail.svg"],
    link: "https://ielts-game.vercel.app/",
    demoVideo: "https://ielts-game.vercel.app/"
  },
  {
    id: 11,
    title: "",
    des: "",
    img: "/inclusive-ai-gateway.svg",
    thumbnailAlt: "Inclusive AI Trust Gateway policy console thumbnail",
    iconLists: ["/next.svg", "/ts.svg", "/vercel.svg"],
    link: "https://inclusive-ai-trust-gateway-web.vercel.app/",
    demoVideo: "https://inclusive-ai-trust-gateway-web.vercel.app/",
    status: "Live, verified 2026-07-16",
    value: "Must-have: strongest current AI governance frontend for the portfolio",
    source: "Required by owner",
    featured: true,
  },
  {
    id: 12,
    title: "",
    des: "",
    img: "/adm-dashboard.svg",
    thumbnailAlt: "Agentic Defense Matrix connected to api.dennisleehappy.org thumbnail",
    iconLists: ["/github.svg", "/ts.svg", "/cloud.svg"],
    link: "https://jest-test-team.github.io/Agentic-Defense-Matrix-ADM/?api=https://api.dennisleehappy.org&gw=https://api.dennisleehappy.org",
    demoVideo: "https://jest-test-team.github.io/Agentic-Defense-Matrix-ADM/?api=https://api.dennisleehappy.org&gw=https://api.dennisleehappy.org",
    status: "Live, verified 2026-07-16",
    value: "Must-have: demonstrates the API gateway through a defensive agent matrix UI",
    source: "Required by owner",
    featured: true,
  },
  {
    id: 13,
    title: "",
    des: "",
    img: "/api-gateway.svg",
    thumbnailAlt: "api.dennisleehappy.org service gateway thumbnail",
    iconLists: ["/cloud.svg", "/ts.svg"],
    link: "https://api.dennisleehappy.org/",
    demoVideo: "https://api.dennisleehappy.org/",
    status: "Live, verified 2026-07-16",
    value: "Supporting service: API and gateway target used by ADM and trust tooling",
    source: "Discovered from required ADM parameters",
  },
];

export const workExperience = [
  {
    id: 1,
    title: "",
    desc: "",
    className: "md:col-span-2",
    thumbnail: "/exp1.svg",
  },
  {
    id: 2,
    title: "",
    desc: "",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp2.svg",
  },
  {
    id: 3,
    title: "",
    desc: "",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp3.svg",
  },
  {
    id: 4,
    title: "",
    desc: "",
    className: "md:col-span-2",
    thumbnail: "/exp4.svg",
  },
];

export const socialMedia = [
  {
    id: 1,
    img: "/git.svg",
    url: "https://github.com/dennislee928",
  },
  {
    id: 2,
    img: "/twit.svg",
    url: "https://x.com/lee66876613",
  },
  {
    id: 3,
    img: "/link.svg",
    url: "https://www.linkedin.com/in/pf-frog-4a3a352a2/",
  },
];
